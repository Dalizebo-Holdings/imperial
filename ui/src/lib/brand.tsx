...
export function generateCSSVariables(theme: { colors: Record<string, string> }): string {
  const vars: string[] = [];
...
// Logo SVG Component
export const LogoMark = ({ width = 32, height = 32 }: { width?: number; height?: number }) => (
  <svg
    width={width}
    height={height}
    viewBox="0 0 32 32"
...
export const LogoWordmark = ({ width = 120, height = 28 }: { width?: number; height?: number }) => (
  <svg
    width={width}
    height={height}
    viewBox="0 0 120 28"
...
export const LogoFull = ({ size = 'medium' }: { size?: 'small' | 'medium' | 'large' }) => {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: s.gap }}>
      <LogoMark width={s.mark} height={s.mark} />
      <LogoWordmark width={s.wordmark} height={s.mark * 0.875} />
    </div>
  );
};
